...
for (patient in patients){
	...
	sc.data = Read10X(data.dir = paste0(sample, '/filtered_feature_bc_matrix'))
	...
	pdf(file = paste0(sample, '_QC.pdf'), height = 10, width = 14)
	...
	pdf(file = paste0(sample, '_clustering.pdf'))
	...
	pdf(file = paste0(sample, '_QC_afterfiltering.pdf'), height = 10, width = 14)
	print(VlnPlot(sc, ncol = 3, pt.size = 0,
		features = features))
	...
	pdf(file = paste0(sample, '_ADT_Markers.pdf'), width = 8)
	...
	pdf(file = paste0(sample, '_GeneSorterTop10.pdf'),
	...
	# Save data
	DefaultAssay(sc) = 'RNA'
	saveRDS(sc, file = paste0(sample, '.rds'))
...
# From https://www.biostars.org/p/383217/
...
for (patient in patients){
	...
	vdj = read.csv(file = paste0(sample_vdj, '/filtered_contig_annotations.csv'))
	...
	# Get clonotype info
	clono = read.csv(file = paste0(sample_vdj, '/clonotypes.csv'))
